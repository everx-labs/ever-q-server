// @flow
import { INDEXES } from '../src/server/data/blockchain';
...
test('Slow Detector', () => {
    ...
    function setCollection(name, type) {
        collectionName = name;
        collectionType = type;
    }

    function isFast(filter, orderBy) {
        return isFastQuery(
            collectionName,
    ...
    expect(isFast({ account_addr: { eq: '1' }, now: { gt: 2 } }, 'now')).toBeTruthy();
    expect(isFast({ workchain_id: { eq: 1 }, now: { gt: 2 } }, 'now')).toBeFalsy();
    expect(isFast({ in_msg: { in: ['1', '2'] }, aborted: { eq: true } })).toBeFalsy();
    ...
    expect(isFast({ in_msg: { eq: '1' }, status: { eq: 3 } })).toBeTruthy();
    ...
    expect(isFast({ gen_utime: { gt: 1 }, workchain_id: { eq: 1 } }, 'gen_utime')).toBeTruthy();
    ...
    expect(isFast({ status: { eq: 5 }, src: { eq: '1' } })).toBeTruthy();
    expect(isFast(
        {
