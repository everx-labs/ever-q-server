...
test('BigUInt', () => {
  ...
  // $FlowFixMe
  expect(convertBigUInt(1, BigInt('0xffffffffffffffff'))).toEqual(
    'fffffffffffffffff',
  )
  ...
  expect(convertBigUInt(2, '0x10000000000000000')).toEqual(
    '1010000000000000000',
  )
  ...
  expect(resolveBigUInt(2, '1010000000000000000')).toEqual(
    '0x10000000000000000',
  )
})
...
test('Negative BigUInt', () => {
  ...
  expect(convertBigUInt(1, '-9223372036854775807')).toEqual(
    '-08000000000000000',
  ) //0X7FFFFFFFFFFFFFFF
  expect(convertBigUInt(1, '-0X7FFFFFFFFFFFFFFF')).toEqual('-08000000000000000')
  expect(convertBigUInt(1, '-18446744073709551615')).toEqual(
    '-00000000000000000',
  )
  expect(convertBigUInt(1, -Number.MAX_SAFE_INTEGER)).toEqual(
    '-2e0000000000000',
  )
  // $FlowFixMe
  expect(convertBigUInt(1, -BigInt('0xffffffffffffffff'))).toEqual(
  ...
  expect(resolveBigUInt(2, '-efeffffffffffffffff')).toEqual(
    '-0x10000000000000000',
  )
})
...
test('Filter test', () => {
  expect(
    Account.test(
      ...
      {
        id: {
          eq: '01d7acd8d454d33c95199346683ef1938d994e6432f1b8a0b11b8eea2556f3b2',
        },
        acc_type: { eq: 3 },
      },
    ),
  ...
  expect(
    Transaction.test(null, { in_msg: null }, { in_msg: { ne: null } }),
  ).toBeFalsy()
  expect(
    Transaction.test(null, { in_msg: null }, { in_msg: { eq: null } }),
  ).toBeTruthy()
...
test('Enum Names', () => {
  ...
  let ql = Message.filterCondition(params, 'doc', {
    msg_type_name: { eq: 'ExtIn' },
  })
  ...
  ql = Message.filterCondition(params, 'doc', {
    msg_type_name: { eq: 'Internal' },
  })
  expect(ql).toEqual('doc.msg_type == @v1')
  expect(params.values.v1).toEqual(0)

  expect(
    Message.test(null, m1, { msg_type_name: { eq: 'ExtIn' } }),
  ).toBeTruthy()
  expect(
    Message.test(null, { msg_type: 0 }, { msg_type_name: { eq: 'Internal' } }),
  ).toBeTruthy()

  params.clear()
  ql = Message.filterCondition(params, 'doc', {
    msg_type_name: { in: ['Internal'] },
  })
  expect(ql).toEqual('doc.msg_type == @v1')
  expect(params.values.v1).toEqual(0)

  expect(
    Message.test(null, m1, { msg_type_name: { in: ['ExtIn'] } }),
  ).toBeTruthy()
})
  ...
  const selected = selectFields(
    {
      ...
      created_by:
        '20f8defd07e745cec86c6b66f71645829d923f4f57cbf75e6f7d3c2423871c6b',
      master: {
