...
function fieldsCanUseIndex(fields: Map<string, QFieldExplanation>, index: QIndexInfo): boolean {
...
function getUsedIndexes(fields: Map<string, QFieldExplanation>, collectionIndexes: QIndexInfo[]): (QIndexInfo[]) | null {
    ...
    if (orderBy.length > 0) {
        if (!orderByCanUseAnyIndex(orderBy, fields, indexes)) {
            return getSlowReason(
                ...
                fields, collectionIndexes, indexes,
            );
        }
    }
