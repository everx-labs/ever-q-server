...
export const config: Config = {
    ...
    messages: compileCollectionConfig({
        alwaysFetchFields: ["chain_order", "src_chain_order", "dst_chain_order"],
        excludeFields: ["hash"],
        ...
        joins: [
            {
                ...
                prefetchQueryBuilder: (parentPath, joinPath, returnExpression) =>
...
export type JoinConfig<TParent, TJoinCollection extends keyof Config> = {
    ...
    prefetchQueryBuilder: (parentPath: string, joinPath: string, returnExpression: string) => string,
    queryBuilder: (path: string, onFieldParam: string, returnExpression: string) => string,
...
export function compileCollectionConfig<TItem>(collection: CollectionConfig<TItem>): CompiledCollectionConfig<TItem> {
    if ("buildReturnExpression" in collection) {
        return collection;
    }

    const joinDepthIsExceededError = (collection.joins) ?
        `Allowed joins depth is exceeded (hint: ${collection.joins.map(j => j.targetField).join(", ")})` :
        "";
    ...
    const returnExpressionBuilder = (
        ...
        for (const join of collection.joins ?? []) {
            const joinSelectionSet = getFieldSelectionSet(selectionSet, join.targetField);
            if (!joinSelectionSet) {
                continue;
            }
            if (maxJoinDepth <= 0) {
            ...
            const returnExression = config[join.joinedCollection].buildReturnExpression(
                joinSelectionSet,
                context,
                maxJoinDepth - 1,
                pathForQuery,
            );
            const query = join.prefetchQueryBuilder(path, pathForQuery, returnExression);
            returnExpressionsOverrides.set(join.targetField, query);
        }

        const shouldBeExcluded = (s: SelectionNode) =>
            s.kind == "Field" && (collection.excludeFields as Maybe<string[]>)?.includes(s.name.value);
    ...
    const joinsFetcher = async (
        ...
        for (const join of collection.joins || []) {
            const joinSelectionSet = getFieldSelectionSet(selectionSet, join.targetField);
            if (!joinSelectionSet || data.every(item => !join.needFetch(item))) {
                continue;
            }
            ...
            data.forEach(item => {
                ...
                if (!onFieldValue) {
                    ...
                    throw QError.create(500, "error during join fetching (no join field)");
                }
            ...
            const returnExpression = config[join.joinedCollection].buildReturnExpression(
                joinSelectionSet,
                context,
                maxJoinDepth - 1,
            ...
            const query = `${join.queryBuilder(pathForQuery, onFieldsParamName, returnExpression)}`;
            const queryResult = await context.services.data.query(
                required(config[join.joinedCollection].qDataCollectionSelector(context).provider),
                {
                    text: query,
                    vars: params.values,
                    orderBy: [],
                    ...
                    for (const parent of parents) {
                        if (onFieldIsArray) {
                            if (parent[join.targetField]) {
                                (parent[join.targetField] as unknown as any[]).push(joinItem);
                            } else {
            ...
            const joined = data.map(d => d[join.targetField]).filter(d => !!d).flat() as any;
            await config[join.joinedCollection].fetchJoins(joined, joinSelectionSet, context, traceSpan, maxJoinDepth - 1);
        }
    }
