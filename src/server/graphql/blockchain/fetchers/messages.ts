...
export async function resolve_account_messages(
    ...
    if (hasInbound) {
        ...
        await prepareChainOrderFilter(args, params, inboundAndFilters, context, "dst_chain_order");
        ...
        if (counterpartiesParamsMap) {
            for (const cpParam of counterpartiesParamsMap.values()) {
                orFilters.push(`${commonFilter} AND doc.msg_type == 0 AND doc.src == @${cpParam}`);
        ...
        queries.push(...orFilters.map(filter =>
            "FOR doc IN messages " +
            `FILTER ${filter} ` +
            `SORT doc.dst_chain_order ${direction == Direction.Backward ? "DESC" : "ASC"} ` +
            `LIMIT ${limit} ` +
    ...
    if (hasOutbound) {
        ...
        await prepareChainOrderFilter(args, params, outboundAndFilters, context, "src_chain_order");
        const returnExpression = returnExpressionBuilder("src_chain_order");
        const commonFilter = outboundAndFilters.join(" AND ");
        const orFilters: string[] = [];
        if (counterpartiesParamsMap) {
            for (const cpParam of counterpartiesParamsMap.values()) {
                orFilters.push(`${commonFilter} AND doc.msg_type == 0 AND doc.dst == @${cpParam}`);
        ...
        queries.push(...orFilters.map(filter =>
            "FOR doc IN messages " +
            `FILTER ${filter} ` +
            `SORT doc.src_chain_order ${direction == Direction.Backward ? "DESC" : "ASC"} ` +
            `LIMIT ${limit} ` +
            `RETURN ${returnExpression}`
        ));
    }

    const query = (queries.length === 1) ? queries[0] :
        `FOR d IN UNION((${queries.join('),(')})) ` +
        `SORT d.account_chain_order ${direction == Direction.Backward ? "DESC" : "ASC"} ` +
        `LIMIT ${limit} ` +
        "RETURN d";
    ...
    return await processPaginatedQueryResult(
        ...
        "account_chain_order",
        async r => {
            await config.messages.fetchJoins(r, selectionSet, context, traceSpan, maxJoinDepth);
        },
