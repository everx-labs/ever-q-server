...
export abstract class AggregationQuery {
    ...
    protected constructor(public fn: AggregationFn, public path: string) {}
...
class BigIntQuery extends AggregationQuery {
    ...
    getQueryText(): string {
        const a = bigIntSumExpr(
            signedBigIntHiPart,
            this.path,
            this.bigIntPrefix,
        )
        const b = bigIntSumExpr(
            signedBigIntLoPart,
            this.path,
            this.bigIntPrefix,
        )
    ...
    reduceResult(values: BigIntParts[]): unknown {
        ...
        return reduced !== undefined
            ? (reduced as { toString(): string }).toString()
            : undefined
    }
}
...
function bigIntStringToDecimalString(
    value: unknown,
    bigIntPrefix: number,
): string | bigint {
...
function signedBigIntPart(
    path: string,
    prefix: number,
    part: (path: string, prefix: number) => string,
): string {
