...
export const Def = {
    ...
    ref(
        nameOrType: string | { [name: string]: TypeDef },
        doc?: SchemaDoc,
    ): ExplicitDef {
        const name =
...
class SchemaParser {
    ...
    namedMembersFromUnorderedDefs<D, S>(
        ...
        return Object.keys(defs).map<SchemaMember<S>>(
            (memberName: string): SchemaMember<S> => {
    ...
    namedMembersFromOrderedDefs<D, S>(
        ...
        defs.forEach((unorderedDefs: UnorderedMembersDef<D>) => {
            this.namedMembersFromUnorderedDefs(
                unorderedDefs,
                mapMember,
            ).forEach((member: SchemaMember<S>) => {
                members.push(member)
            })
        })
        return members
    ...
    typedNamedMembers<D extends TypeDef>(
        memberDefs: MembersDef<D>,
        name: string,
    ): SchemaMember<SchemaType>[] {
        return this.namedMembers<D, SchemaType>(
            memberDefs,
            (memberName: string, memberDef: D): SchemaType => {
                return this.parseTypeDef(
                    memberDef,
                    combineName(name, memberName),
                )
            },
        )
    }

    typedNamedOrderedMembers<D extends TypeDef>(
        memberDefs: OrderedMembersDef<D>,
        name: string,
    ): SchemaMember<SchemaType>[] {
        return this.namedMembersFromOrderedDefs<D, SchemaType>(
            memberDefs,
            (memberName: string, memberDef: D): SchemaType => {
                return this.parseTypeDef(
                    memberDef,
    ...
    parseTypeDef(def: TypeDef, name: string): SchemaType {
        ...
        if (scalarType) {
            ...
            type[scalarType.substr(1)] = (def as Record<string, TypeDef>)[
                scalarType
            ]
            ...
            type.class = {
                types: classDef.types
                    ? this.typedNamedMembers(classDef.types, name)
                    : [],
                fields: classDef.fields
                    ? this.typedNamedMembers(classDef.fields, name)
                    : [],
                functions: classDef.functions
                    ? this.namedMembers(
                          classDef.functions,
            ...
            Object.keys(def as StructDef).forEach(key => {
                if (!isReservedKey(key)) {
                    filteredDef[key] = (def as StructDef)[key] as
                        | TypeDef
                        | StructDef
                }
            })
