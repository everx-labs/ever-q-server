...
async function updateCollection(name: string, indexes: QIndexInfo[], arango: Database) {
    ...
    for (const existing of existingIndexes) {
        if (!indexes.find(x => sameFields(x.fields, existing.fields))) {
            console.log(`${name}: remove index [${existing.id}] on [${existing.fields.join(",")}]`);
    ...
    for (const required of indexes) {
        if (!existingIndexes.find(x => sameFields(x.fields, required.fields))) {
            console.log(`${name}: create index on [${required.fields.join(",")}]`);
...
async function updateDb(config: { server: string, name: string, auth?: string }) {
